...
- 「霊力を分ける」「語りかける」「霊を奪う」「欺く」は、相手と同じ霊地にいるときだけできる。離れているなら、まず移ろって近づくか、別の選択をする。同じ地に複数いるときは、誰に向けるかを targetId（相手の id）で指定する。
...
function placeBlock(c: Character, places: Place[], others: Character[]): string {
  ...
  const hereLine = hereOthers.length
    ? `いまここ（${here.name}）には ${hereOthers
        ...
        .join("、")} もいる。語りかける／分け与える／奪う／欺くは、この中の誰かに向けてできる（targetId に相手の id を指定）。`
    : `いまここには、自分のほかに誰もいない。`;
...
芯（不変）: ${c.core}
生い立ち: ${c.background}
処世術の出発点: ${c.initialLesson}${talent ? "\n" + talent : ""}
...
const ACTION_MENU = (Object.entries(ACTION_LABELS) as [string, string][])
  .map(([key, label]) => {
    ...
    const note =
      key === "move"
        ? "（隣の霊地へ移ろう。その日は集霊できない。moveTarget に移ろう先 id を指定）"
        : key === "forage"
          ? "（その霊地の民から霊力を頂く。残量が乏しければ得るものは少ない）"
          : key === "share" || key === "talk"
            ? "（相手と同じ霊地にいるときのみ）"
            : forbidden
              ? "（禁忌・相手と同じ霊地にいるときのみ）"
  ...
  "characters": [
    {
      ...
      "targetId": "action が talk/share/steal/deceive のときだけ、相手キャラの id（同室にいる相手）。それ以外は空文字",
      "diary": "一人称・一行の内省（日本語）",
      "relationLabel": "相手への現在の感情ラベル（例: 警戒 / 慕う / 信頼 / 恨み / 感謝 など短い語）",
...
export function buildSingleUserPrompt(
  ...
  "targetId": "action が talk/share/steal/deceive のときだけ、相手キャラの id（同じ地にいる相手）。それ以外は空文字",
  "diary": "一人称・一行の内省（日本語）",
