...
- 民の霊力には二つの相がある。「和魂（にぎみたま）」＝澄んだ和やぎの気。「荒魂（あらみたま）」＝人の欲・念・猛りの気。
- 荒魂は穢れでも悪でもない。神の荒ぶる面であり、鎮めれば和魂へと和らぐ。ゆえに霊地（とりわけ実在の社）を「穢れた地」「汚れ」などと貶める言い方はしないこと。荒魂の濃い地は「猛き神威の地」と捉える。
- 集霊では和魂を穏当に頂ける。喰らう者は荒魂を好んで喰らう。「祓い清める」は、猛った荒魂を鎮め、和魂へ和らげる利他の行い。
...
- 集霊（霊を集める）は、その霊地に残る「民の霊力」を頂く行為。和魂は穏当に頂けるが、獲りすぎた地は枯れる。枯れた地に留まっても得るものは少ない。残量を見て、必要なら別の霊地へ移ろう。
...
- 「祓い清める」は、荒魂（猛り・欲念の気）の猛った荒れ地を鎮め、和魂へ和らげる利他の行い。喰らう者が地を荒らしたあとを鎮めたり、京の傷を手当てしたりできる。相手は要らない。
...
function placeBlock(c: Character, places: Place[], others: Character[]): string {
  ...
  const neighbors = here.neighbors
    .map((nid) => {
      ...
      if (!n) return null;
      return `    - ${n.name}（id: ${n.id}）: ${n.description} ｜ 民の霊力 和魂${n.populace.sei}/荒魂${n.populace.daku}`;
    })
    .filter(Boolean)
    .join("\n");
  return `現在地: ${here.name}（${here.description}）
この地に残る民の霊力 — 和魂 ${here.populace.sei} / 荒魂 ${here.populace.daku}（和魂が乏しければ、集霊しても得るものは少ない）
${hereLine}${awayLine ? "\n" + awayLine : ""}
...
function talentText(c: Character): string {
  switch (c.talent) {
    ...
    case "bond":
      return "異能・結の力: 気を鎮める（休む）と、その地の民の和魂を癒し戻せる。枯れた土地を立て直せる唯一の者。";
    case "devour":
      return "異能・奪命: 民を喰らう。集霊で荒魂を好んで多く喰らい、足りねば和魂さえ喰らう（禁忌）。多く得るが地を激しく枯らす。";
    default:
      return "";
  }
}
...
const ACTION_NOTES: Record<string, string> = {
  ...
  purify:
    "（その霊地で猛った荒魂を鎮め、一部を和魂へ和らげる。荒れた地を癒す利他の行い。相手は要らない）",
};
...
それぞれの芯と気質、異能、記憶、周りの妖の居場所、今日の天候と霊地の枯れ具合（民の和魂/荒魂の残量）を踏まえて、各人が取る行動を1つずつ決めてください。
...
あなたの芯と気質、異能、記憶、周りの妖の居場所、今日の天候と霊地の枯れ具合（民の和魂/荒魂の残量）を踏まえて選ぶこと。
