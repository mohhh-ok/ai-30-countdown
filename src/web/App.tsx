  ...
  return (
    <div className="title">
      <div className="title-bar">
        ...
        {/* GitHub リポジトリへのリンク（言語ピッカーの左隣。ml-auto で右側へ寄せる） */}
        <a
          className="ml-auto flex items-center text-[color:var(--text)] hover:text-[color:var(--accent)]"
          href="https://github.com/mohhh-ok/ai-30-countdown"
          target="_blank"
        ...
        {/* 🌐 はセレクト枠内の左端に重ね表示（option ラベルには付けない）。
            pointer-events-none でクリックは下の select に透過する */}
        <span className="relative flex items-center">
          <span
            className="pointer-events-none absolute left-2 text-[15px] leading-none"
            aria-hidden="true"
          >
            🌐
          </span>
          <select
            className="lang-select pl-7"
            value={lang}
