...
- **決定論パート（TSコード）**: 負荷 −8 / 霊力収支 / パラメータの 0–100 クランプ・±5上限 / 死亡判定 / 段階しきい値 / 天候抽選 / 記憶管理 / 移動の妥当性。
...
| 行動 | `Action` | 効果 | 備考 |
|---|---|---|---|
| 集霊する | `forage` | 通常日 +12 / 不作日 +5 | 一人で霊力を得る |
| 気を鎮める | `rest` | +6 | 何もせず回復 |
...
| 祓い清める | `purify` | 自分 −2 / 濁霊を清霊へ還す | 相手不要。結界力を貯める |
...
| 場所 | ID | 特徴 | 集霊（通常/不作） |
|---|---|---|---|
| 鴨川の河原 | `kamogawa` | 街なかの水辺・ハブ | 12 / 5 |
| 大原の隠れ里 | `ohara` | 豊かな里・畑 | 16 / 7 |
| 貴船の渓 | `kibune` | 山奥・安定だが控えめ | 9 / 7 |
| 嵐山の竹林 | `arashiyama` | 西郊・やや多め | 14 / 5 |
| 伏見の稲荷 | `fushimi` | 実り大だが不作に弱い博打 | 18 / 1 |

- 「霊力を分ける」「語りかける」「霊を奪う」は**相手と同じ場所にいるときだけ**。
- 「寄り添う」は**離れた相手にも向けられる**。相手のいる方へ一歩近づく（その日は集霊できない）。同じ場所なら傍にいて絆を深める。
- 「祓い清める」は相手不要。その地の濁霊を祓い、一部を清霊へ還す。
