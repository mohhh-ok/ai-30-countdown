...
- **回避＝クリア＝fin（スキル方式）**: ハルが回帰をまたいで積んだ**結界力 `wardPower`** が**猛威度 `CLIMAX_MENACE`（既定30）以上なら大禍を祓い退ける**。届かなければ京は呑まれ、全員が倒れて回帰する。
- **クリア＝回帰の輪を断つ（fin）**: 回帰は「ハルが力尽きる」ことで起きるのだから、大禍を祓って生き延びた周には**巻き戻る理由がない**。クリアした瞬間に `world.finished = true` となり（`campaign.recordTick`）、年代記に最終周（`endKind="cleared"`）を刻んで**物語はそこで完結する**。次の周は立ち上がらず、サーバの自走ワーカーも停止する（`server.ts`／再起動しても fin 済み run は動かない）。観客ビューには幕引き（fin バナー）が出る。
