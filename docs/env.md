...
| `CLAUDE_CODE_OAUTH_TOKEN` | — | **Production (Railway) only.** Subscription OAuth token issued by `claude setup-token`; lets `claude -p` authenticate headlessly with the Max plan. Never put `ANTHROPIC_API_KEY` in the environment instead — that would switch claude to pay-as-you-go API billing (see [deploy.md](deploy.md) / [llm-backend.md](llm-backend.md)) |
